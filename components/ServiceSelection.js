...
function ServiceSelection({ onSelect, selectedService }) {
    ...
    const [categoriaActiva, setCategoriaActiva] = React.useState('General');
    ...
    const categorias = React.useMemo(() => {
        const nombres = services.map(service => normalizarCategoria(service.categoria));
    ...
    const serviciosFiltrados = React.useMemo(() => {
        return services.filter(service => normalizarCategoria(service.categoria) === categoriaActiva);
    ...
    React.useEffect(() => {
        if (selectedService) {
            setCategoriaActiva(normalizarCategoria(selectedService.categoria));
            return;
        }
        ...
        return (
            <div className="space-y-4 animate-fade-in">
                ...
                <div className="text-center py-8">
                    ...
                    <p className="text-purple-400 mt-4">Cargando servicios...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="space-y-4 animate-fade-in">
            ...
            {services.length === 0 ? (
                <div className="text-center p-8 bg-white/80 backdrop-blur-sm rounded-xl border border-purple-200">
                    ...
                    <p className="text-xs text-purple-400 mt-2">La administradora debe cargar servicios primero</p>
                </div>
            ) : (
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {serviciosFiltrados.map(service => (
                            <button
                                key={service.id}
                                onClick={() => onSelect({...service, categoria: normalizarCategoria(service.categoria)})}
                                className={`
                                    ...
                                    ${selectedService?.id === service.id 
                                        ...
                                        : 'border-purple-200 bg-white/80 backdrop-blur-sm hover:border-purple-400 hover:bg-purple-50/50 hover:shadow-sm'}
                                `}
                            >
                                <div className="flex justify-between items-start">
                                    ...
                                    <div className="flex flex-col items-end gap-1 ml-4">
                                        ...
                                        <span className="flex items-center text-purple-600 text-xs bg-purple-50 px-2 py-1 rounded-full border border-purple-200">
